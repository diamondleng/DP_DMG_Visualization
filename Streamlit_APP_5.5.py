...
# Sidebar selection
view_selection = st.sidebar.radio("Select View:", ["History Match Evaluation", "Pressure Difference", "Pressure Gradient", "PG (Constant Compressibility Test)"])

if view_selection == "History Match Evaluation":
    file1_path = "3.31.25_sx10_3.xlsx"
    file2_path = "Calibration_DMG_InjPermWell_pfAll_MDF_TT_3.31.xlsx"

    well_pressure_df = pd.read_excel(file1_path, usecols=['Name', 'Date', 'Value'])
    injection_df_full = pd.read_excel(file2_path, usecols=['API_10', 'Injection Date', 'BHP_MDF_T', 'Assigned Disposal Formation Name'])

    well_pressure_df.rename(columns={'Name': 'Well_Name', 'Value': 'BHP Simulation'}, inplace=True)
    injection_df_full.rename(columns={'API_10': 'Well_Name', 'BHP_MDF_T': 'BHP B3', 'Assigned Disposal Formation Name': 'ADFN'}, inplace=True)

    injection_df_filtered = injection_df_full
    well_pressure_df['Date'] = pd.to_datetime(well_pressure_df['Date'])
    injection_df_filtered['Injection Date'] = pd.to_datetime(injection_df_filtered['Injection Date'])

    merged_df_filtered = pd.merge(
        well_pressure_df,
        injection_df_filtered,
        left_on=['Well_Name', 'Date'],
        right_on=['Well_Name', 'Injection Date'],
        how='inner'
    )
    merged_df_filtered.dropna(subset=['BHP Simulation', 'BHP B3'], inplace=True)

    rmse = np.sqrt(mean_squared_error(merged_df_filtered['BHP Simulation'], merged_df_filtered['BHP B3']))

    fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(16, 10), gridspec_kw={'height_ratios': [1, 1]})
    upper_lim = 7000
    lower_lim = 0

    # Scatter Plot
    axes[0, 0].scatter(merged_df_filtered['BHP Simulation'], merged_df_filtered['BHP B3'], alpha=0.6, label='Data Points')
    axes[0, 0].plot([lower_lim, upper_lim], [lower_lim, upper_lim], color='red', linestyle='--', linewidth=2, label='x=y line')
    axes[0, 0].set_xlim(lower_lim, upper_lim)
    axes[0, 0].set_ylim(lower_lim, upper_lim)
    axes[0, 0].set_xlabel('Simulation BHP, psi')
    axes[0, 0].set_ylabel('Field BHP from B3 WHP, psi')
    axes[0, 0].set_title('Scatter Plot: Simulation vs B3 Data')
    axes[0, 0].grid(True)
    axes[0, 0].text(0.05, 0.8, f'RMSE: {rmse:.2f} psi', transform=axes[0, 0].transAxes, fontsize=12, color='blue', bbox=dict(facecolor='white', alpha=0.7))
    axes[0, 0].legend()

    # Density Plot
    x_bins = np.arange(lower_lim, upper_lim, 100)
    y_bins = np.arange(lower_lim, upper_lim, 100)
    hist = axes[0, 1].hist2d(merged_df_filtered['BHP Simulation'], merged_df_filtered['BHP B3'], bins=[x_bins, y_bins], cmap='Reds', alpha=1)
    axes[0, 1].plot([lower_lim, upper_lim], [lower_lim, upper_lim], color='red', linestyle='--', linewidth=2)
    axes[0, 1].set_xlim(lower_lim, upper_lim)
    axes[0, 1].set_ylim(lower_lim, upper_lim)
    axes[0, 1].set_xlabel('Simulation BHP, psi')
    axes[0, 1].set_ylabel('Field BHP from B3 WHP, psi')
    axes[0, 1].set_title('Density Plot: Simulation vs B3 Data')
    axes[0, 1].grid(True)
    fig.colorbar(hist[3], ax=axes[0, 1], fraction=0.046, pad=0.04).set_label('Density in each 100x100 bin')

    # Best-matching well plot
    best_well = merged_df_filtered.groupby('Well_Name').apply(lambda df: np.sqrt(mean_squared_error(df['BHP Simulation'], df['BHP B3']))).idxmin()
    best_df = merged_df_filtered[merged_df_filtered['Well_Name'] == best_well]
    best_df_sorted = best_df.sort_values('Date')

    # Pressure comparison plot for best well
    axes[1, 0].scatter(best_df_sorted['Date'], best_df_sorted['BHP B3'], color='blue', label='BHP B3 (Measured)')
    axes[1, 0].plot(best_df_sorted['Date'], best_df_sorted['BHP Simulation'], color='orange', label='BHP Simulation')
    axes[1, 0].set_title(f'Best Match Well: {best_well}')
    axes[1, 0].set_ylabel('BHP (psi)')
    axes[1, 0].legend()
    axes[1, 0].grid(True)
    ...
    # Continue with other views as already implemented (DP, PG, Compressibility Test)
